...
pub fn evaluate_hand(cards: &[Card; 7]) -> HandStrength {
    ...
    for s in 0..4 { if suit_counts[s] >= 5 { flush_suit = Some(s); break; } }
    ...
    if let Some((quad, kicker)) = detect_quads(&rank_counts) {
        return HandStrength { category: Category::FourOfAKind, kickers: [quad, kicker, 0, 0, 0] };
    }

    // Full house
    if let Some((trip, pair)) = detect_full_house(&rank_counts) {
        return HandStrength { category: Category::FullHouse, kickers: [trip, pair, 0, 0, 0] };
    }
    ...
    if let Some(s) = flush_suit {
        ...
        let mut k = [0u8; 5];
        for i in 0..5 { k[i] = ranks[i]; }
        return HandStrength { category: Category::Flush, kickers: k };
    }

    // Straight
    let mut uniq: Vec<u8> = Vec::new();
    for r in (2..=14).rev() {
        if rank_counts[r as usize] > 0 { uniq.push(r as u8); }
    }
    uniq.sort_unstable(); uniq.dedup();
    if let Some(high) = detect_straight_high(&uniq) {
        return HandStrength { category: Category::Straight, kickers: [high, 0, 0, 0, 0] };
    }
    ...
    if let Some(t) = trip_ranks.first().copied() {
        ...
        return HandStrength { category: Category::ThreeOfAKind, kickers: k };
    }
    if pair_ranks.len() >= 2 {
        let mut prs = pair_ranks.clone();
        prs.sort_unstable(); prs.reverse();
        let high = prs[0]; let low = prs[1];
        let mut k = [high, low, 0, 0, 0];
        let mut rest = singles.clone();
        rest.sort_unstable_by(|a,b| b.cmp(a));
        k[2] = *rest.get(0).unwrap_or(&0);
        return HandStrength { category: Category::TwoPair, kickers: k };
    }
    if let Some(p) = pair_ranks.first().copied() {
        let mut k = [p, 0, 0, 0, 0];
        let mut rest = singles.clone();
        rest.sort_unstable_by(|a,b| b.cmp(a));
        for i in 0..3 { k[i+1] = *rest.get(i).unwrap_or(&0); }
        return HandStrength { category: Category::OnePair, kickers: k };
    }
    ...
    let mut k = [0u8; 5];
    for i in 0..5 { k[i] = *highs.get(i).unwrap_or(&0); }
    HandStrength { category: Category::HighCard, kickers: k }
}
...
fn detect_straight_high(sorted_unique_ranks: &[u8]) -> Option<u8> {
    ...
    for i in 1..w.len() {
        if w[i] == w[i-1] + 1 {
            run += 1;
            if run >= 5 { best_high = w[i]; }
        } else if w[i] != w[i-1] { // break in sequence
            run = 1;
        }
    }
    if best_high == 0 { None }
...
fn detect_quads(rank_counts: &[u8; 15]) -> Option<(u8, u8)> {
    ...
    for r in (2..=14).rev() {
        ...
        if c == 4 { quad = r as u8; break; }
    }
    if quad == 0 { return None; }
    for r in (2..=14).rev() {
        if r as u8 != quad && rank_counts[r as usize] > 0 { kicker = r as u8; break; }
    }
    Some((quad, kicker))
}
...
fn detect_full_house(rank_counts: &[u8; 15]) -> Option<(u8,u8)> {
    ...
    if !pairs.is_empty() { return Some((trips[0], pairs[0])); }
    None
}
