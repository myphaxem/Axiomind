use axm_engine::logger::{HandRecord, ShowdownInfo, ActionRecord, Street};
...
fn showdown_info_serializes() {
    let rec = HandRecord {
        ...
        seed: Some(1),
        actions: vec![ActionRecord{ player_id: 0, street: Street::River, action: A::Check }],
        board: vec![Card{ suit: S::Clubs, rank: R::Ace }],
        result: Some("p0".into()),
        ts: None,
        meta: None,
        showdown: Some(ShowdownInfo{ winners: vec![0], notes: Some("kicker A".into()) }),
    };
