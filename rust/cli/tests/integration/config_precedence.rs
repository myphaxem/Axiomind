...
fn i1_cfg_shows_defaults_for_adaptive_and_ai_version() {
    ...
    let out = res.stdout;
    assert!(out.contains("\"adaptive\": true"), "defaults should set adaptive=true: {}", out);
    assert!(out.contains("\"ai_version\": \"latest\""), "defaults should set ai_version=latest: {}", out);
}

#[test]
fn i2_precedence_cli_over_env_over_file_for_seed_and_ai() {
    let tfm = TempFileManager::new().unwrap();
    let cfg_path = tfm.create_file("axm.toml", "seed = 456\nai_version = \"v1\"\nadaptive = false\n").unwrap();
    ...
    assert_eq!(r1.stdout, r2.stdout, "same seed should produce identical RNG output");
}
...
fn i3_seed_default_is_non_deterministic() {
    ...
    let a = cli.run(&["rng"]);
    let b = cli.run(&["rng"]);
    assert_ne!(a.stdout, b.stdout, "rng without --seed should be non-deterministic");
}
