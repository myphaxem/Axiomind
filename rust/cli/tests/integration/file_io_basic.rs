...
fn c1_replay_requires_input_arg() {
    ...
    assert!(err.contains("required") || err.contains("usage"), "stderr should indicate missing required arg: {}", res.stderr);
}
...
fn c2_replay_speed_validation() {
    ...
    assert_ne!(res.exit_code, 0);
    assert!(res.stderr.to_lowercase().contains("speed"), "stderr should mention speed violation: {}", res.stderr);
}
