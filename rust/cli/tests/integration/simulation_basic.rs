...
fn d4_sim_outputs_identical_for_same_seed_and_level() {
    let tfm = TempFileManager::new().expect("tfm");
    let path1 = tfm.create_file("sim/run1.jsonl", "").expect("run1");
    let path2 = tfm.create_file("sim/run2.jsonl", "").expect("run2");
    ...
    let res1 = cli.run(&[
        "sim", "--hands", "3", "--seed", "7", "--level", "2", "--output", &out1,
    ]);
    assert_eq!(res1.exit_code, 0, "first sim failed: {}", res1.stderr);

    let res2 = cli.run(&[
        "sim", "--hands", "3", "--seed", "7", "--level", "2", "--output", &out2,
    ]);
