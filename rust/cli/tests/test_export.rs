...
fn mk_jsonl(name: &str, n: usize) -> PathBuf {
    ...
    for i in 0..n { let mut r = base.clone(); r.hand_id = format!("20250102-{:06}", i+1); s.push_str(&serde_json::to_string(&r).unwrap()); s.push('\n'); }
    fs::write(&p, s).unwrap();
    p
}
...
fn export_to_csv() {
    ...
    let code = run(["axm","export","--input", input.to_string_lossy().as_ref(), "--format","csv","--output", output.to_string_lossy().as_ref()], &mut out, &mut err);
    assert_eq!(code, 0);
...
fn export_to_json_array() {
    ...
    let code = run(["axm","export","--input", input.to_string_lossy().as_ref(), "--format","json","--output", output.to_string_lossy().as_ref()], &mut out, &mut err);
    assert_eq!(code, 0);
