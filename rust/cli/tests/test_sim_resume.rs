...
fn sim_gracefully_saves_partial_and_resumes() {
    ...
    let code1 = run(["axm","sim","--hands","5","--seed","3","--output", path.to_string_lossy().as_ref()], &mut out1, &mut err1);
    assert_ne!(code1, 0);
    ...
    let code2 = run(["axm","sim","--hands","5","--seed","3","--resume", path.to_string_lossy().as_ref()], &mut out2, &mut err2);
    assert_eq!(code2, 0);
